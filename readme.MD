...
- 구성
    - 노드(node) :  
        ...
        - 프롬프트 실행, 파이쎤 코드 실행, api 호출, ...
        - 특정 수행 목적을 가진 에이전트(어떤 작업을 수행하는 단위, 규모는 제각각)
    - 엣지(edge) : 
        - 노드와 노드를 `연결`하는 경로
        - A 노드의 작업이 끝나면 B 노드로 이동하여 작업을 시작해라 -> `규칙` 규정
        - 조건에 따라 특정 노드로 이동하여 작업 진행 -> `조건`부 엣지
    - 상태관리(state) :
        - 그래프 전체를 관통하는 공유메모리
        - 노드에서 작업이 완료되고 특정 값, 상태를 공유메모리에 업데이트하면 다른 노드가 이를 엑세스하여 진행 가능.
- 사용 이유
    - 체인 단방향 -
        - lopping(되돌아가가)이 어려움
    ...
    - 역할 분담
        - 에이전트 규모에 따라서는 협업필요함 -> 단방향을 절차적 진행되기 떄문에 협업하는 구조가 수직 구조임
...
# Agent, A2A(Agent - to - Agent) 아키텍쳐
- 개요
    - 어떤 작업은 하나의 거대한 모델(llm)이 모두 처리 하는 것이 아님
    - 전문화된 페르소나를 가진 에이전트들이 서로 결과물을 입력받아서 작업을 고도화 하는 방ㅇ식(협업)
...
- 특징
    - 순차적 협업 패턴
        - 에이전트 정의
            - Developer Agent : '구현'에 집중하는 프롬프트를 가짐. 창의적, 생산적 등등 역할
            - Reviewer Agent  : '비판', '분석', '평가'에 집중하는 프롬프트를 가짐. 논리적, 점증적인 등등 학할
            - 체크 : Claude는 성능이 너무 좋아서  -> 한번에 완료될 수도 있으므로 -> 주니어 개발자로 설정
        - 메세지 전달
            - 다른 에이전트의 출력이 다른 에이전트의 입력이 됨
        - 피드백 루프
            - 일을 지시하고 마무리 되는것이 아니라, 리뷰 결과에 따라 행동 결정
            - 카운트 부여 -> 반복 횟수를 제한 할 수 있음
...
- 전체 아키텍쳐
    ...
    사용자 -> 프롬프트 -> 랭체인/랭그래프 -> LLM 판단 -> 도구사용필요 -> 도구목록선택 -> 외부도구(MCP 요청, MCP Client) -> Adapter(파이썬 -> JSON-RPC 2.0) -> MCP Server -> 비지니스 로직 -> 응답 -> (JSON-RPC 2.0 -> 파이썬) -> MCP Client 응답 -> LLM 응답 -> 다음 노드(단계) 진행 -> ...
